...
# 安装依赖
# 建议加上 ca-certificates，保证 https 下载和查询 IP 可用
apk add --no-cache openssl iproute2 iptables ip6tables net-tools ca-certificates
...
########################################
# 下面是新增：自动生成 V2RayN 节点链接 #
########################################

# 获取服务器公网 IP（尝试两个服务，失败就给个占位）
PUBLIC_IP=$(
...
# 节点名中的空格简单转一下（v2rayN 也不严格要求，这里做个最基本处理）
